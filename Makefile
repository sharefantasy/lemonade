...
build:
	go build -ldflags "-X github.com/sharefantasy/lemonade/lemon.Version=$(VERSION)"

install:
	go install -ldflags "-X github.com/sharefantasy/lemonade/lemon.Version=$(VERSION)"

release:
	gox --arch 'amd64 386' --os 'windows linux darwin' --output "dist/{{.Dir}}_{{.OS}}_{{.Arch}}/{{.Dir}}" -ldflags "-X github.com/sharefantasy/lemonade/lemon.Version=$(VERSION)"
